...
class PageEditPhotoOrderRequest(BaseModel):
    ordered_asset_rel_ids: list[UUID]
...
class PageAPIHandler(RouteHandler):
    def register_routes(self) -> None:
        ...
        self.route(
            "/api/pages/{page_id}/edit_photo_orders",
            "page_edit_photo_orders",
            ["POST"],
        )
    ...
    @enforce_response_model
    async def page_edit_photo_orders(
        self,
        request: Request,
        page_id: UUID,
        payload: PageEditPhotoOrderRequest,
    ) -> PagesOverviewResponse:
        async with self.app.new_db_session() as db_session:
            request_context = await self.get_request_context(request)
            page = await self.get_page_assert_owned_by_user(
            ...
            # 1. Load all page_asset_rels
            rels = await DALPagesAssetsRel.list_all(
                db_session,
                filters={"page_id": (FilterOp.EQ, page.id)},
            )
            rels_by_id = {rel.id: rel for rel in rels}

            # 2. Validate IDs
            unknown_ids = [
                rel_id
                for rel_id in payload.ordered_asset_rel_ids
                if rel_id not in rels_by_id
            ]
            if unknown_ids:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid asset_rel_ids for page: {unknown_ids}",
                )

            # 3. Apply new order_index
            async with safe_commit(db_session):
                await DALPagesAssetsRel.update_many_by_ids(
                    db_session,
                    {
                        rel_id: DAOPagesAssetsRelUpdate(order_index=index)
                        for index, rel_id in enumerate(payload.ordered_asset_rel_ids)
                    },
                )

            # 4. Render full response
            full_response = await PagesFullResponse.rendered_from_daos(
                [page],
                db_session,
